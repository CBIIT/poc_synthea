...
if __name__ == '__main__':
    ...
    if postgres_pw is None:
        ...
        sys.exit(1)
    '''
    cts_api_key = os.environ.get('CTS_API_KEY')
    if not cts_api_key:
        print('CTS_API_KEY environment variable must be set', file=sys.stderr)
        sys.exit(1)

#    search = trial_search.TrialSearch(args.db_host, args.db_port, args.db_name, args.db_user, args.db_password)
    search = trial_search.TrialSearch(cts_api_key)

    @app.post('/')
    def consume_bundle():
        bundle = Bundle(request.json)
        return search.search(bundle)
